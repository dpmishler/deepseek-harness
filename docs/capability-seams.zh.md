...
flowchart LR
  ...
  pkg_speech["speech"]
  svc_speech["ctx.speech<br/>Speech (STT/TTS) provider registry"]
  ...
  pkg_speech_deepgram_flux_stt --> svc_speech
  pkg_speech_deepgram_flux_tts --> svc_speech
...
| `ctx.speech` | `seam` | [`speech`](../packages/speech/speech) | [`speech-deepgram-flux-stt`](../packages/speech/speech-deepgram-flux-stt), [`speech-deepgram-flux-tts`](../packages/speech/speech-deepgram-flux-tts) | [`speech-agent`](../packages/speech/speech-agent) | - | STT/TTS 提供方通过显式 id 注册到同一个 ctx.speech seam（无自动选择）；speech-agent 的 TurnController 是唯一的 Consumer，将两个方向关联成一次支持打断的对话。 |
