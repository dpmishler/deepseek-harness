 ...
 * and TTS provider registered on `ctx.voice`. Types here name concepts, not
...
export interface SttOpenOptions {
  /** Provider id registered via {@link VoiceRuntime.registerSttProvider}. */
...
export interface SttSession {
   ...
   * transport owns backpressure. Throws {@link VoiceError} (`SESSION_CLOSED`)
...
/** Provider-wire adapter for {@link SttSession}. Register with `ctx.voice.registerSttProvider(id, provider)`. */
...
export interface TtsOpenOptions {
  /** Provider id registered via {@link VoiceRuntime.registerTtsProvider}. */
  readonly provider: string
...
export interface TtsSession {
   ...
   * responsibility. Throws {@link VoiceError} (`SESSION_CLOSED`) once closed.
   ...
   *   of the session (not the current turn). Required to receive
   *   `textSpoken`/`textRemaining` on the reply; each call's offset must
   *   exceed the previous interrupt's — {@link PlaybackClock} maintains this
   *   session-wide counter for a caller driving real audio playback.
   */
...
/** Provider-wire adapter for {@link TtsSession}. Register with `ctx.voice.registerTtsProvider(id, provider)`. */
