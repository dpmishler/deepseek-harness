 ...
 * (WebSocket messages, timers) into the pull-based `for await` event streams
 * every voice session exposes.
 * @module @deepseek-ai/dsh-voice/async-event-queue
 */
 ...
 * concurrent consumer, matching every session's single `events` stream.
 * `push` after `end`/`fail` is silently dropped: a transport's late callback
 * racing its own shutdown must not resurrect a finished stream.
 */
export class AsyncEventQueue<T> implements AsyncIterable<T> {
  ...
  /** Signal a terminal failure: the buffer is discarded and the next (or current) `next()` rejects. */
