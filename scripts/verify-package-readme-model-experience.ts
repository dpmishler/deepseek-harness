...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/speech/speech': { kind: 'none', reason: 'The STT/TTS provider registry only opens sessions and forwards audio/text; a consumer such as dsh-speech-agent owns any model-facing surface.' },
  'packages/speech/speech-deepgram-flux-stt': { kind: 'none', reason: 'The Deepgram Flux STT adapter only opens sessions; the model-facing surface belongs to whatever consumer turns a transcript into a user/message.' },
  'packages/speech/speech-deepgram-flux-tts': { kind: 'none', reason: 'The Deepgram Flux TTS adapter only opens sessions; it renders whatever text a caller streams in.' },
  'packages/speech/speech-agent': { kind: 'none', reason: 'TurnController only orchestrates already-open STT/TTS sessions; it builds no system prompt or tool schema of its own.' },
